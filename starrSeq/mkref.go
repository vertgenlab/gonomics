...
func Mkref(upstream, constructs, downstream, output string) {
	var bds []bed.Bed
	var bd bed.Bed
	var ref fasta.Fasta
	var idx int
	o := fileio.EasyCreate(output)
	ref.Name = "chrSS"
	...
	for i := range c {
		...
		bd = bed.Bed{Chrom: "chrSS", ChromStart: idx, ChromEnd: idx + len(c[i].Seq), Name: c[i].Name, FieldsInitialized: 4}
