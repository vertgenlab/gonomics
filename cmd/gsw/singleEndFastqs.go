...
func GswToGiraf(ref *genomeGraph.GenomeGraph, readOne string, output string, threads int, seedLen int, stepSize int, scoreMatrix [][]int64) {
	...
	start := time.Now()
	for i := 0; i < threads; i++ {
		go genomeGraph.RoutineFqToGiraf(ref, seedHash, settings.TileSize, settings.StepSize, settings.ScoreMatrix, fastqPipe, girafPipe, &wgAlign)
	}
	wgWrite.Add(1)
...
func GswToSam(ref *genomeGraph.GenomeGraph, readOne string, output string, threads int, seedLen int, stepSize int, scoreMatrix [][]int64, header sam.Header) {
	...
	start := time.Now()
	for i := 0; i < threads; i++ {
		go genomeGraph.RoutineGirafToSamSingle(ref, seedHash, settings.TileSize, settings.StepSize, settings.ScoreMatrix, fastqPipe, samPipe, &wgAlign)
	}
	wgWrite.Add(1)
