...
func main() {
	...
	if *filter {
		...
		defer output.Close()
		vcf.WriteHeaderDev(output.File, header)
        ...
        if strings.Contains(vcfFilter, "AS") {
        	for record := range vcfPipe {
	        	if vcf.ASFilter(record, vcf.MapNameToIndex(dict.HapIdx, AaAa), vcf.MapNameToIndex(dict.HapIdx, AAaa)) {
	        		vcf.WriteVcf(output.File, record)
	               	vcf.ViewGenotypeVcf(record)
	        	}
	        }
	     }
	     if strings.Contains(vcfFilter, "wrong"){
	        for record := range vcfPipe {
