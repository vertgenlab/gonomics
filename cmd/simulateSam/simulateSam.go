...
func simulateSam(s Settings) {
	...
	for i := range ref {
		simulate.IlluminaPairedSam(ref[i].Name, ref[i].Seq, readsPerContig[i], s.ReadLength, s.FragmentLength, s.FragmentStdDev, s.FlatError, s.AncientErrorRate, flatBinomialAlias, ancientBinomialAlias, s.GeometricParam, out, bw, bamOutput, deaminationDistributionSlice, seed)
	}
