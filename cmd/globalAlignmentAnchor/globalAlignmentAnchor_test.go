...
func TestGlobalAlignmentAnchorTests(t *testing.T) {
	...
	for i, test := range globalAlignmentAnchorTests {
		...
		if test.in_maf == "" {
			...
			globalAlignmentAnchor(test.in_maf, test.species1, test.species2, test.species1_genome, test.species2_genome, test.gapSizeProductLimit, test.chrMap_filename, test.out_filename_prefix)
