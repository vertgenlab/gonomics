...
func VisualizeUsage(VisualizeFlags *flag.FlagSet) {
	fmt.Printf("pFaTools Visualize - Provides human-readable SOMETHING HERE from a given pFa.\n" +
		"Keyword 'END' for the end argument makes a visualiztion until the end of the pfasta.\n" +
		"Usage:\n" +
...
func parseVisualizeArgs() {
	...
	var chrom string = flag.String("chrom", "", "Specify the name of the sequence to display.")
	...
	if strings.ToLower(Visualiseflags.Arg(2)) == "end" {
		endOfAlignment = true
	} else {
		endPos = parse.StringToInt(Visualiseflags.Arg(2))
	}
...
func pFaVisualize(s VisualizeSettings) {
	browser.pFaVisualizer(s.Infile, s.OutDir, s.Start, s.End, s.EndOfAlignment, s.SigFigs, s.DecimalPlaces, s.LineLength, s.Chrom)
}
