...
func usage() {
	fmt.Print(
		"ontologyEnrichment will assign regions in the input data to their closest gene provided in the gene file input either " +
			...
			"each query region will be assigned a GO term and then enrichment for each GO term as it relates to the input data will " +
			"be output along with the p-value of that enrichment calculated by approximating the binomial with a normal distribution." +
...
func main() {
	...
	var termEnrichments *bool = flag.Bool("termEnrichments", true, "If user doesn't want to output the GO term enrichments and p-values for a set of input data, set to false.")
	flag.Usage = usage
