...
func alleleToVcf(allele *Allele, p float64, altBase dna.Base, indelSlicePos int, warnings []string) vcf.Vcf {
	...
	case dna.A:
		...
		answer.Alt = []string{"A"}
		answer.Samples = []vcf.Sample{{FormatData: []string{fmt.Sprint(allele.Count.Counts), fmt.Sprint(allele.Count.BaseAF + allele.Count.BaseAR)}}}
	case dna.C:
		answer.Ref = dna.BaseToString(allele.Count.Ref)
		answer.Alt = []string{"C"}
		answer.Samples = []vcf.Sample{{FormatData: []string{fmt.Sprint(allele.Count.Counts), fmt.Sprint(allele.Count.BaseCF + allele.Count.BaseCR)}}}
	case dna.G:
		answer.Ref = dna.BaseToString(allele.Count.Ref)
		answer.Alt = []string{"G"}
		answer.Samples = []vcf.Sample{{FormatData: []string{fmt.Sprint(allele.Count.Counts), fmt.Sprint(allele.Count.BaseGF + allele.Count.BaseGR)}}}
	case dna.T:
		answer.Ref = dna.BaseToString(allele.Count.Ref)
		answer.Alt = []string{"T"}
		answer.Samples = []vcf.Sample{{FormatData: []string{fmt.Sprint(allele.Count.Counts), fmt.Sprint(allele.Count.BaseTF + allele.Count.BaseTR)}}}
	case dna.Gap:
		...
		answer.Samples = []vcf.Sample{{FormatData: []string{fmt.Sprint(allele.Count.Counts), fmt.Sprint(allele.Count.Indel[indelSlicePos].CountF + allele.Count.Indel[indelSlicePos].CountR)}}}
	}
	return answer
}
