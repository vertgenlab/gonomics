...
func TestBedGcContent(t *testing.T) {
	var err error
	for _, v := range BedGcContentTests {
		bedGcContent(v.BedFile, v.FaFile, v.OutFile, v.MultiFaMode, v.Species)
		if !fileio.AreEqual(v.OutFile, v.ExpectedFile) {
			t.Errorf("Error in bedGcContent. Output did not match expected.")
		} else {
