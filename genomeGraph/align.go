...
func GraphSmithWatermanMemPool(gg *GenomeGraph, read fastq.FastqBig, seedHash map[uint64][]uint64, seedLen int, stepSize int, scoreMatrix [][]int, m [][]int, trace [][]rune, memoryPool **SeedDev) sam.Sam {
	...
	//printSeedDev(seeds)
	//for currSeed = seeds; currSeed != nil && seedCouldBeBetter(int(currSeed.TotalLength), bestScore, perfectScore, int(len(read.Seq)), 100, 90, -196, -296); currSeed = currSeed.Next {
	for i := 0; i < len(seeds) && seedCouldBeBetter(int(seeds[i].TotalLength), bestScore, perfectScore, int(len(read.Seq)), 100, 90, -196, -296); i++ {
		currSeed = seeds[i]
		...
		if currScore > bestScore {
			...
			/*if &gg.Nodes[bestPath[0]].Info != nil {
				...
				//currBest.Pos += int(gg.Nodes[bestPath[0]].Info.Start) - 1
			}*/
...
func perfectMatchBig(read fastq.FastqBig, scoreMatrix [][]int) int {
...
func scoreSeedSeq(seq []dna.Base, start uint32, end uint32, scoreMatrix [][]int) int {
	var score int = 0
	for i := start; i < end; i++ {
...
func scoreSeedFastqBig(seed *SeedDev, read fastq.FastqBig, scoreMatrix [][]int) int {
	var score int = 0
...
func scoreSeedPart(seed *SeedDev, read fastq.Fastq, scoreMatrix [][]int) int {
	var score int = 0
	for i := seed.QueryStart; i < seed.QueryStart+seed.Length; i++ {
		score += scoreMatrix[read.Seq[i]][read.Seq[i]]
	}
...
func scoreSeed(seed *SeedDev, read fastq.Fastq, scoreMatrix [][]int) int {
	var score int = 0
...
//perfect match
func perfectMatch(read fastq.Fastq, scoreMatrix [][]int) int {
	var perfectScore int = 0
	for i := 0; i < len(read.Seq); i++ {
		perfectScore += scoreMatrix[read.Seq[i]][read.Seq[i]]
	}
