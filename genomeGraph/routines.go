...
func RoutineFqToGiraf(gg *GenomeGraph, seedHash map[uint64][]uint64, seedLen int, stepSize int, scoreMatrix [][]int64, inputChan <-chan fastq.FastqBig, outputChan chan<- giraf.Giraf, wg *sync.WaitGroup) {
	...
	for read := range inputChan {
		outputChan <- *GraphSmithWatermanToGiraf(gg, read, seedHash, config, matrix, &seedPool, &dnaPool, scorekeeper, dynamicKeeper, seedBuildHelper)
	}
	wg.Done()
}
...
func RoutineFqPairToGiraf(gg *GenomeGraph, seedHash map[uint64][]uint64, config *GraphSettings, matrix *sync.Pool, input <-chan fastq.PairedEndBig, output chan<- giraf.GirafPair, wg *sync.WaitGroup) {
	...
	for read := range input {
		output <- WrapPairGiraf(gg, read, seedHash, config, matrix, &seedPool, &dnaPool, scorekeeper, dynamicKeeper, seedBuildHelper)
	}
	wg.Done()
}
...
func RoutineGirafToSamSingle(gg *GenomeGraph, seedHash map[uint64][]uint64, seedLen int, stepSize int, scoreMatrix [][]int64, inputChan <-chan fastq.FastqBig, outputChan chan<- sam.Sam, wg *sync.WaitGroup) {
	...
	config := DefaultAlignment()
	for read := range inputChan {
		outputChan <- GirafToSam(GraphSmithWatermanToGiraf(gg, read, seedHash, config, matrix, &seedPool, &dnaPool, scorekeeper, dynamicKeeper, seedBuildHelper))
	}
	wg.Done()
}
...
func RoutineGirafToSam(gg *GenomeGraph, seedHash map[uint64][]uint64, seedLen int, stepSize int, scoreMatrix [][]int64, input <-chan fastq.PairedEndBig, output chan<- sam.Sam, wg *sync.WaitGroup) {
	...
	for read := range input {
		pair = GirafPairToSam(WrapPairGiraf(gg, read, seedHash, config, matrix, &seedPool, &dnaPool, scorekeeper, dynamicKeeper, seedBuildHelper))
		output <- pair.Fwd
		output <- pair.Rev
	}
	wg.Done()
