...
func GraphSmithWatermanToGiraf(gg *GenomeGraph, read fastq.FastqBig, seedHash map[uint64][]uint64, config *GraphSettings, matrixTrace *sync.Pool, seedPool *sync.Pool, sk scoreKeeper, seedBuildHelper *seedHelper) *giraf.Giraf {
	...
	pool := matrixTrace.Get().(*MatrixMemoryPool)
	defer matrixTrace.Put(pool)

	resetScoreKeeper(sk)
	readLen := len(read.Seq)
	sk.perfectScore = perfectMatchBig(read, config.ScoreMatrix)
	sk.extension = int(sk.perfectScore/600) + readLen
	...
	for i := 0; i < len(seeds.Hits) && seedCouldBeBetter(int64(seeds.Hits[i].TotalLength), int64(currBest.AlnScore), sk.perfectScore, int64(len(read.Seq)), config); i++ {
		...
		if int(sk.currSeed.TotalLength) == readLen {
		...
		if sk.currScore > int64(currBest.AlnScore) {
			...
			currBest.Cigar = cigar.SoftClipBases(currBest.QStart, readLen, cigar.CatByteCigar(cigar.AddCigarByte(sk.leftAlignment, cigar.ByteCigar{RunLen: uint16(sk.currSeed.TotalLength), Op: cigar.Match}), sk.rightAlignment))
...
func WrapPairGiraf(gg *GenomeGraph, fq fastq.PairedEndBig, seedHash map[uint64][]uint64, config *GraphSettings, memoryPool *sync.Pool, seedPool *sync.Pool, sk scoreKeeper, seedBuildHelper *seedHelper) giraf.GirafPair {

	var mappedPair giraf.GirafPair = giraf.GirafPair{
		Fwd: *GraphSmithWatermanToGiraf(gg, fq.Fwd, seedHash, config, memoryPool, seedPool, sk, seedBuildHelper),
		Rev: *GraphSmithWatermanToGiraf(gg, fq.Rev, seedHash, config, memoryPool, seedPool, sk, seedBuildHelper),
	}
