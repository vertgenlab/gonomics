...
func GraphSmithWatermanToGiraf(gg *GenomeGraph, read fastq.FastqBig, seedHash map[uint64][]uint64, settings *GraphSettings, seedPool *sync.Pool, sk scoreKeeper, seedBuildHelper *seedHelper, memory *sync.Pool) *giraf.Giraf {
...
func WrapPairGiraf(gg *GenomeGraph, fq fastq.PairedEndBig, seedHash map[uint64][]uint64, settings *GraphSettings, seedPool *sync.Pool, sk scoreKeeper, seedBuildHelper *seedHelper, memory *sync.Pool) giraf.GirafPair {
	var mappedPair giraf.GirafPair = giraf.GirafPair{
		Fwd: *GraphSmithWatermanToGiraf(gg, fq.Fwd, seedHash, settings, seedPool, sk, seedBuildHelper, memory),
		Rev: *GraphSmithWatermanToGiraf(gg, fq.Rev, seedHash, settings, seedPool, sk, seedBuildHelper, memory),
	}
