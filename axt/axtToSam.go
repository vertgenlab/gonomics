...
func AxtToSam(axtFmt *Axt) sam.Aln {
	var answer sam.Aln = sam.Aln{
		...
		RNext: "*",
		PNext: 0,
		TLen:  int32(axtFmt.REnd - axtFmt.RStart), //Could leave at zero or make TLen be the length of alignment, start and end (not sure if i can get target length from an axt)
