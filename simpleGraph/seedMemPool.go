...
func RoutinePathSeed(gg *SimpleGraph, seedHash map[uint64][]uint64, seedLen int, stepSize int, scoreMatrix [][]int64, input <-chan FastqGsw, output chan<- GirafGsw, wg *sync.WaitGroup) {
	...
	for read := range input {
		output <- wrapPathSeedGsw(gg, read, seedHash, seedLen, stepSize, &matrix, scoreMatrix, scorekeeper, dynamicKeeper)
	}
	wg.Done()
}
func wrapPathSeedGsw(gg *SimpleGraph, fq FastqGsw, seedHash map[uint64][]uint64, seedLen int, stepSize int, matrix *MatrixAln, scoreMatrix [][]int64, sk scoreKeeper, dynamicScore dynamicScoreKeeper) GirafGsw {
	var mappedPair GirafGsw = GirafGsw{
		ReadOne: pathSeedDevGraphSmithWaterman(gg, fq.ReadOne, seedHash, seedLen, stepSize, matrix, scoreMatrix, sk, dynamicScore),
		ReadTwo: pathSeedDevGraphSmithWaterman(gg, fq.ReadTwo, seedHash, seedLen, stepSize, matrix, scoreMatrix, sk, dynamicScore),
	}
...
func pathSeedDevGraphSmithWaterman(gg *SimpleGraph, read fastq.FastqBig, seedHash map[uint64][]uint64, seedLen int, stepSize int, matrix *MatrixAln, scoreMatrix [][]int64, sk scoreKeeper, dynamicScore dynamicScoreKeeper) giraf.Giraf {
	...
	seeds := GetSeedPathHits(seedHash, gg.Nodes, &read, seedLen, sk.perfectScore, scoreMatrix)
	...
	for i := 0; i < len(seeds) && seedCouldBeBetter(int64(seeds[i].Len), int64(currBest.AlnScore), sk.perfectScore, int64(len(read.Seq)), 100, 90, -196, -296); i++ {
		...
		if int(currSeed.Len) == len(currSeq) {
			...
			sk.leftAlignment, sk.leftScore, sk.minTarget, sk.minQuery, sk.leftPath = LeftAlignTraversal(gg.Nodes[currSeed.TNodes[0]], sk.leftSeq, currSeed.TStart, sk.leftPath, sk.extension-currSeed.Len, currSeq[:currSeed.QStart], scoreMatrix, matrix, dynamicScore)
			sk.rightAlignment, sk.rightScore, sk.maxTarget, sk.maxQuery, sk.rightPath = RightAlignTraversal(gg.Nodes[currSeed.TNodes[len(currSeed.TNodes)-1]], sk.rightSeq, currSeed.TEnd, sk.rightPath, sk.extension-currSeed.Len, currSeq[currSeed.QStart+currSeed.Len:], scoreMatrix, matrix, dynamicScore)
...
func GetSeedPathHits(seedHash map[uint64][]uint64, nodes []*Node, read *fastq.FastqBig, seedLen int, perfectScore int64, scoreMatrix [][]int64) []pathSeed {
	...
	for readStart := 0; readStart < len(read.Seq)-seedLen+1; readStart++ {
		...
		for _, codedNodeCoord = range currHits {
			...
			tempSeeds = ExtendRightPath(nodes[nodeIdx], read, readStart-(leftMatches-1), int(nodePos)-(leftMatches-1), true)
		...
		for _, codedNodeCoord = range currHits {
			...
			tempSeeds = ExtendRightPath(nodes[nodeIdx], read, readStart-(leftMatches-1), int(nodePos)-(leftMatches-1), false)
...
func ExtendRightPath(node *Node, read *fastq.FastqBig, readStart int, nodeStart int, posStrand bool) []pathSeed {
	...
	if readStart+rightMatches < len(read.Seq) && nodeStart+rightMatches == node.SeqTwoBit.Len && len(node.Next) != 0 {
		for i = 0; i < len(node.Next); i++ {
			...
			nextParts = ExtendRightPath(node.Next[i].Dest, read, readStart+rightMatches, 0, posStrand)
			// if we aligned into the next node, make a seed for this node and point it to the next one
			for j = 0; j < len(nextParts); j++ {

				currNode.TNodes = append(currNode.TNodes, nextParts[j].TNodes...)
				currNode.TEnd = nextParts[j].TEnd
				currNode.Len = nextParts[j].Len
