...
// for lowMem4 checkerboard implementation, have 2 functions initialize Scoring
	// and Trace data structures separately
...
func AffineGap(alpha []dna.Base, beta []dna.Base, scores [][]int64, gapOpen int64, gapExtend int64) (int64, []Cigar) {
	...
	//Call the customized checkersize function
	score_highest, route := AffineGap_customizeCheckersize(alpha, beta, scores, gapOpen, gapExtend, checkersize_i, checkersize_j)
...
func AffineGap_customizeCheckersize(alpha []dna.Base, beta []dna.Base, scores [][]int64, gapOpen int64, gapExtend int64, checkersize_i int, checkersize_j int) (int64, []Cigar) { //input=same as AffineGap_step1 for now, output=route
	...
	//i_inChecker_max: the max i-index (row-index) in the current checkerboard during Step 2 (fillTraceback),
		//either checkersize-1 or a smaller number, e.g. if the alpha/beta sequence to be aligned is not divisible
		// into perfect checkerboards, and the remainder checkerboard is not completely filled
	//j_inChecker_max: the max j-index (column-index) in the current checkerboard during Step 2 (fillTraceback)
	//k_inChecker_max: the k-index (out of 0,1,2) of the max i and j in the current checkerboard
		//during Step 2 (fillTraceback)
	//i_inChecker_min: the min i-index (row-index) in the current checkerboard during Step 3 (writeCigar),
		//either 0 or another number, e.g. if cigar route leaves the checkerboard at a position that is
		// not i_inChecker==0, j_inChecker==0
	//j_inChecker_min: the min j-index (column-index) in the current checkerboard during Step 3 (writeCigar)
	//k_inChecker_min: the k-index (out of 0,1,2) of the min i and j in the current checkerboard during
		//Step 3 (writeCigar)
	//note that k1 amd k2 (coordinates for checkerboads) are not related to k_inChecker_max
		//and k_inChecker_min (out of 0,1,2). Also note that k_inCkecer_max and k_inChecker_min do not determine
		// traceback path (e.g. determine the next k1 and k2 to go to), but influence cigar route
	...
	for k1, k2 = int((score_highest_i-1)/checkersize_i), int((score_highest_j-1)/checkersize_j); k1 >= 0 && k2 >= 0; { //use a function of score_highest_i, score_highest_j, and checkersize to initialize the right k1 and k2, go to the correct checkerboard to start traceback
		...
		//Step 2: for a checkerboard, fill traceback, as well as find the max i and j
			//when the checkerboard traceback ends. Since the trace matrix was initialized above,
			// pass it back and forth to recycle the memory instead of creating another trace
			// in Step 2 in every iteration
		//for affineGap, step 2 is almost the same as constGap, just have 3 dimensions of k (0,1,2),
			//use gapOpen and gapExtend instead of gapPen, add k_inChecker_max
		...
		//Step 3: for a checkerboard, use traceback to write cigar (update route and routeIdx),
		 	//as well as find the min i and j when the checkerboard cigar ends
		//for affineGap, step 3 is almost the same as constGap, just have 3 dimensions of k (0,1,2),
		 	//add k_inChecker_min
		...
		//Use Step 3's i_inChecker_min and j_inChecker_min to find the next checkerboard
			//to go to and where to start in that checkerboard (update k1, k2)
	...
	//Step 4: write the last cigar entry. Since this step doesn't need special features for affineGap,
		//reuse constGap function
	//This step is necessary because the row i=0 and the column j=0 are always stored in trace_prep,
...
func fillTraceback_affineGap(alpha []dna.Base, beta []dna.Base, scores [][]int64, gapOpen int64, gapExtend int64, checkersize_i int, checkersize_j int, score_highest_i int, score_highest_j int, trace_prep_i [][][]int64, trace_prep_j [][][]int64, k1 int, k2 int, i_inChecker_min_Previous int, j_inChecker_min_Previous int, trace [][][]ColType) ([][][]ColType, int, int, ColType) {
	...
	//i_inChecker and j_inChecker are internal (within checkerboard) versions of i and j,
		//e.g. for checkersize=3, i_inChecker can take on the values 0, 1, 2
	...
	if i_inChecker_min_Previous >= 0 {
		//use the i_inChecker_min_Previous from the previous k1/k2 loop iteration's Step 3 writeCigar
			//to find the i_max at which to stop filling the trace matrix
		i_max = checkersize_i*k1 + 1 + i_inChecker_min_Previous
	} else {
		//otherwise, since no restriction from previous k1/k2 loop Step 3 writeCigar,
			//find the minimum value between "where the next checkerboard starts" and "the i of the highest score",
			//and that is where to stop filing the trace matrix
...
func writeCigar_affineGap(trace [][][]ColType, i_inChecker_max_Previous int, j_inChecker_max_Previous int, k_inChecker_max_Previous ColType, route []Cigar, routeIdx_current int, i_inChecker_min_Previous int, j_inChecker_min_Previous int, k_inChecker_min_Previous ColType) ([]Cigar, int, int, int, ColType) {
	...
	//use the i_inChecker_min_Previous from the previous k1/k2 loop iteration's Step 3 writeCigar
		//to find the i_inChecker_max at which to start writing cigar
	if i_inChecker_min_Previous >= 0 {
