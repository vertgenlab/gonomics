...
func ConstGap_testing2(alpha []dna.Base, beta []dna.Base, scores [][]int64, gapPen int64) (int64, []Cigar) {
	...
	for idx := 0; idx < len(trace_prep_i); idx++ {
		...
		trace_prep_i[idx] = make([]int64, len(beta)+1) //e.g. trace_prep_i is 4 rows * 15 columns
	}
	for idx := 0; idx < len(trace_prep_j); idx++ {
		trace_prep_j[idx] = make([]int64, len(alpha)+1) //e.g. trace_prep_j is 5 columns * 11 rows
	}
	...
	for i = 0; i < mColumn; i++ {
		...
		if i % checkersize == 0 { //if i/checkersize remainder is 0, e.g. i=0,3,6,9
	...
	fmt.Printf("score_highest: %d\n", score_highest) //TODO: remove after debugging

	//start traceback
	var k1, k2, i_inChecker, j_inChecker, routeIdx int //added k1 to cut row into checkersize, and k2 to column into checkersize, internal/within checkerboard i and j, e.g. i_inChecker and j_inChecker can take on values 0,1,2
	route := make([]Cigar, 1)
	for k1, k2 = len(trace_prep_i)-1, len(trace_prep_j)-1; k1 >= 0 || k2 >= 0; { //while loop
		...
		mRowPrevious = trace_prep_i[k1] //initialize the mRowPrevious of the checkerboard. e.g. k1=3, mRowPrevious is now row i=9 (when k1=0, mRowPrevious is row i=0)
		mRowCurrent[checkersize*k2] = trace_prep_j[k2][checkersize*k1+1] //e.g. i=k1*checkersize=9, k2=4, mRowCurrent is i=10, mRowCurrent[3*4=12]=trace_prep_j[4][9+1=10] (when k2=0, for each mRowCurrent, will fill j=0 entries)
		...
		for i = checkersize*k1+1; i < checkersize*(k1+1) && i < mColumn; i++ { //check not only checkersize but also the last chunk <checkersize, break loop if break either condition, e.g. k1=3, start from i=3*3+1=10 since i=3 is prep, need to satisfy i<3*4=12 && i<mColumn=11, so stop at (including) i=10
			...
			for j = checkersize*k2+1; j < checkersize*(k2+1) && j < len(mRowCurrent); j++ { //e.g. k2=4, start from j=4*3+1=13 since j=12 is prep, need to satisfy j<3*5=15 && j<len(mRowCurrent)=15, so stop at (including) j=14
				j_inChecker = (j-1) % checkersize //Updates with each j
				...
				//fmt.Printf("k1, k2, i, j, i_inChecker, j_inChecker, alpha[i-1], beta[j-1], mRowCurrent[j], trace[i_inChecker][j_inChecker]: %d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n", k1, k2, i, j, i_inChecker, j_inChecker, alpha[i-1], beta[j-1], mRowCurrent[j], trace[i_inChecker][j_inChecker]) //TODO: remove after debugging
			}
		...
		route_tmp := make([]Cigar, 1)
		for i_inChecker, j_inChecker, routeIdx = checkersize-1, checkersize-1, 0; i_inChecker > 0 && j_inChecker > 0; { //this used to be || but now I think we need && because when either internal index (i or j) reaches 0, need to switch to another checkerboard
			...
			if route_tmp[routeIdx].RunLength == 0 {
				route_tmp[routeIdx].RunLength = 1
				route_tmp[routeIdx].Op = trace[i_inChecker][j_inChecker]
			} else if route_tmp[routeIdx].Op == trace[i_inChecker][j_inChecker] {
				route_tmp[routeIdx].RunLength += 1
			} else {
				route_tmp = append(route_tmp, Cigar{RunLength: 1, Op: trace[i_inChecker][j_inChecker]})
				routeIdx++
			}

			if i_inChecker != 0 && j_inChecker != 0 { //since my condition is i_inChecker > 0 && j_inChecker > 0, this if may not be necessary
			...
			} else if i_inChecker == 0 && j_inChecker == 0 { //this block was moved up to prevent index out of range
				// go to the next checkerboard with lower i, lower j
				k1, k2 = k1-1, k2-1
				i_inChecker = checkersize-1 //restore i_inChecker and j_inChecker to prevent infinite loop?
				j_inChecker = checkersize-1
			} else if i_inChecker == 0 { //but j_inChecker != 0
				// go to the next checkerboard with lower i, same j
				k1 -= 1
				i_inChecker = checkersize-1
				j_inChecker = checkersize-1
		...
		reverseCigar(route_tmp)
		route = append(route, route_tmp...) //use ... to append a []Cigar to another []Cigar

		//switch to a different block
		//move this block of code upwards, since earlier i_inChecker and j_inChecker have already changed.
		//if i_inChecker == 0 && j_inChecker == 0 {
			// go to the next checkerboard with lower i, lower j
			//k1, k2 = k1-1, k2-1
		//} else if i_inChecker == 0 { //but j_inChecker != 0
			// go to the next checkerboard with lower i, same j
