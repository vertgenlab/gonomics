...
func ConstGap(alpha []dna.Base, beta []dna.Base, scores [][]int, gapPen int) (int, []Cigar) {
...
func ConstGap_customizeCheckersize(alpha []dna.Base, beta []dna.Base, scores [][]int, gapPen int, checkersize_i int, checkersize_j int) (int, []Cigar) {
...
func highestScore(alpha []dna.Base, beta []dna.Base, scores [][]int, gapPen int, checkersize_i int, checkersize_j int) (int, int, int, [][]int, [][]int) {

	mRowCurrent := make([]int, len(beta)+1)
	mRowPrevious := make([]int, len(beta)+1)
	var mColumn int = len(alpha) + 1
	trace_prep_i := make([][]int, int(len(alpha)/checkersize_i)+1) //trace_prep_i saves all rows (i) that are needed to initialize checkerboard tracebacks
	trace_prep_j := make([][]int, int(len(beta)/checkersize_j)+1)  //trace_prep_j saves all columns (j) that are needed to initialize checkerboard tracebacks
	for idx := 0; idx < len(trace_prep_i); idx++ {
		trace_prep_i[idx] = make([]int, len(beta)+1)
	}
	for idx := 0; idx < len(trace_prep_j); idx++ {
		trace_prep_j[idx] = make([]int, len(alpha)+1)
	}
	var i, j int
...
func fillTraceback(alpha []dna.Base, beta []dna.Base, scores [][]int, gapPen int, checkersize_i int, checkersize_j int, score_highest_i int, score_highest_j int, trace_prep_i [][]int, trace_prep_j [][]int, k1 int, k2 int, i_inChecker_min_Previous int, j_inChecker_min_Previous int, trace [][]ColType) ([][]ColType, int, int) {
