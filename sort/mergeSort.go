...
func GoExternalMergeSort[E any](data <-chan E, recordsPerTmpFile int, tmpFileDir string, less func(a, b E) bool) <-chan E {
	out := make(chan E, 1000)
	go mergeSort(data, less, out, recordsPerTmpFile, tmpFileDir)
	return out
}

func mergeSort[E any](data <-chan E, less func(a, b E) bool, out chan<- E, recordsPerTmpFile int, tmpFileDir string) {
	var err error
	...
	tmpFiles = chunkData(data, recordsPerTmpFile, less, tmpFileDir)
...
func chunkData[E any](data <-chan E, recordsPerChunk int, less func(a, b E) bool, tmpFileDir string) []*os.File {
	...
	for v := range data {
		...
		if len(currChunk) == recordsPerChunk {
			...
			tmpFiles = append(tmpFiles, writeTmpFile(currChunk, tmpFileDir))
...
// gob decoder.
func writeTmpFile[E any](chunk []E, tmpFileDir string) *os.File {
	file, err := os.CreateTemp(tmpFileDir, "sort_chunk_")
	defer file.Close()
	exception.PanicOnErr(err)
