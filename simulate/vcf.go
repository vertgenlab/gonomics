...
func SimulateVcf(alpha float64, n int, k int, outFile string) {
	...
	var genotypes [][]vcf.GenomeSample = popgen.SimulateGenotype(alpha, n, k)

	var current *vcf.Vcf
	//for each segregating site, we make a vcf entry and write out
	for i := 0; i < k; i++ {
		//most fields are hardcoded but can be filled in later
		current = &vcf.Vcf{Chr: "chr1", Pos: 1, Id: ".", Ref: "A", Alt: "T", Qual: 100, Filter: ".", Info: ".", Format: ".", Notes: vcf.GenotypeToStringNew(genotypes[i])}
